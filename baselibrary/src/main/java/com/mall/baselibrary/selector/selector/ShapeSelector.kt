package com.mall.baselibrary.selector.selector
...
class ShapeSelector private constructor() : ISelectorUtil<Drawable, View> {
     ...
     */
    fun dashLine(dashLineWidth: Int, @ColorRes dashLineColorResId: Int, dashWidth: Float, dashGap: Float): ShapeSelector {
    ...
    fun dashLine(dashLineWidth: Int, dashLineColor: String, dashWidth: Float, dashGap: Float): ShapeSelector {
        this.isDashLine = true
        this.dashLineWidth = dashLineWidth
        this.dashWidth = dashWidth
        this.dashGap = dashGap
    ...
    private fun getItemShape(shape: Int, solidColor: Int, lineColor: Int, dashLineColor: Int): GradientDrawable {
    ...
    private fun isGradientCreate(drawable: GradientDrawable) {
        if (isGradient) {
            when (gradientType) {
                GradientDrawable.LINEAR_GRADIENT -> drawable.orientation = createGradientOrientation()//设置线性渐变方向
